    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h2 className="text-4xl font-bold text-gray-800 mb-2">Company Data</h2>
        ...
        {data.company_summary && (
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-6 animate-fadeIn">
            <div className="border-l-4 border-indigo-600 pl-4 mb-6">
              <h3 className="text-2xl font-bold text-gray-800">Company Summary</h3>
            </div>
            <p className="text-gray-600 leading-relaxed text-lg">{data.company_summary}</p>
          </div>
        )}
        ...
        {(data.LinkedIn || data.Twitter || data.Facebook || data.Instagram || data.YouTube) && (
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-6 animate-fadeIn">
            <div className="border-l-4 border-green-600 pl-4 mb-6">
              <h3 className="text-2xl font-bold text-gray-800">Social Accounts</h3>
            </div>
        ...
        {(data.Twitter_Followers || data.Facebook_Followers || data.Instagram_Followers || data.YouTube_Subscribers || data.linkdein_followers) && (
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-6 animate-fadeIn">
            <div className="border-l-4 border-purple-600 pl-4 mb-6">
              <h3 className="text-2xl font-bold text-gray-800">Followers & Following Details</h3>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {data.Issues && data.Issues.length > 0 && (
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-6 animate-fadeIn">
            <div className="border-l-4 border-red-600 pl-4 mb-6">
              <h3 className="text-2xl font-bold text-gray-800">Pain Points</h3>
              <p className="text-gray-600 text-sm mt-1">Click here to know pain points company</p>
            </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-red-50">
                    <th className="px-6 py-4 text-left text-sm font-bold text-red-800 border-b-2 border-red-200 w-16">
                      #
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-red-800 border-b-2 border-red-200">
                      Issue
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-red-800 border-b-2 border-red-200">
                      Problem / Pain Points
                ...
                <tbody>
                  {data.Issues.map((issue, index) => (
                    ...
                    >
                      <td className="px-6 py-4 text-sm text-gray-600 border-b border-gray-200 font-semibold">
                        {index + 1}
                      </td>
...
function SocialLink({ platform, url, color }: { platform: string; url: string; color: string }) {
  return (
    <a
      ...
      className={`${color} text-white px-6 py-3 rounded-lg hover:opacity-90 transition-opacity flex items-center gap-2 shadow-md`}
    >
      <span>{platform}</span>
